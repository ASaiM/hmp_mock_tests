...
The ASaiM framework produces accurate and precise taxonomic assignations, wide functional results (gene families, pathways, GO slim terms) and relations between taxonomic and functional results, in few hours on a commodity computer (around 5h30 for almost 1.4 millions of raw metagenomic sequences). Hence, these analyses validate the relevance of ASaiM framework and its workflows.

# Data

[Two mock community samples](https://www.ebi.ac.uk/metagenomics/projects/SRP004311) for Human Microbiome Project (HMP) are available on [*EBI metagenomics* database](https://www.ebi.ac.uk/metagenomics/). Both samples contain a genomic mixture of 22 microbial strains with differences in abundances of the strains (Table \ref{expected_species}). In first sample ([SRR072232](https://www.ebi.ac.uk/metagenomics/projects/SRP004311/samples/SRS121012/runs/SRR072232/results/versions/1.0)), the rRNA operon counts (targeted abundances) vary by up to four orders of magnitude per strains (Table \ref{expected_species}), whereas second sample ([SRR072233](https://www.ebi.ac.uk/metagenomics/projects/SRP004311/samples/SRS121011/runs/SRR072233/results/versions/1.0)) contains equimolar ribosomal rRNA operon counts per strain (Table \ref{expected_species}). Both samples were sequenced using 454 GS FLX Titanium to get 1,225,169 and 1,386,198 raw metagenomic sequences for respectively the first dataset (SRR072232) and the second dataset (SRR072233).
...
Targeted abundances of strains are based on 16S copies in PCR added for each strains to build the community. Before analyzing *EBI metagenomics* and ASaiM results, raw reads are mapped on reference genomes of expected strains using BWA [@li_fast_2009;@li_fast_2010] (using default parameters). Hence, we compute "exact" abundances of expected strains in metagenomic datasets, after experiments and sequencing (*i.e.* not based on expected rRNA operon counts).

Composition based on mapping-based relative abundances of strains are similar to composition of targeted relative abundance (Figure \ref{mapping_comparison}) with Bray-Curtis dissimilarity scores equal to 0.338 for SRR02232 and 0.479 for SRR072233. For SRR02232, similar variations in relative abundances between species are then observed using mapping and rRNA operon counts (Figure \ref{mapping_comparison}). Observations are different for SRR072233 (Figure \ref{mapping_comparison}): targeted abundances are identical for all species, but unexpected variations are observed for mapping based abundances. These differences between targeted abundances and mapping-based abundances can be explained by copy number variation of 16S genes between species. Indeed, the targeted abundances are based on 16S copy number. And, the number of 16S gene copies is not identical for all targeted strains (from 1 for *Candida albicans* to 14 for *Clostridium beijerinckii*). Hence, even with identical targeted abundances of 16S copies (*e.g.* for SRR072233), the abundance of corresponding strains may differ. It will then induce a difference between relative abundance based on mapping reads on whole genome and the targeted relative abundance based on rRNA operon counts because of 16S gene copy variation number. GO FURTHER + coverage !
